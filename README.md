...
本项目用于对教材图 5.2 所示的 **6 节点、8 巷道**通风网络进行数值解算：在给定各巷道风阻 `R` 与入口总风量 `Q_total` 的条件下，采用 **Hardy Cross 回路迭代法**求解各巷道风量 `Q(i)`，并输出压降与物理校核结果。
...
- **回路压降平衡（能量守恒）**
  ...
  其中正负号由回路方向与巷道方向一致/相反决定。

### 3) 求解算法（Hardy Cross）

对每个独立回路计算回路压降闭合差并修正风量：
...
其中，$s_{ki}$ 为巷道 `i` 在回路 `k` 中的方向符号（`+1/-1`）。每轮回路修正后，脚本会按 `Q1 + Q2 = Q_total` 对全网风量做一次比例归一化，防止数值漂移。

> 说明：如果你的 Markdown 渲染器不支持行内公式（单 `$...$`），本 README 仅使用 `$$...$$` 的块级公式以保证可识别。
